...
| 2026-04-23 | **Improve Simpanan import UI**: replace button group with dropdown selector for 6 simpanan types (Pokok, Wajib, Sibuhar, Simapan, Sihat, Sihar) plus Penarikan options, clear icons, 2-column grid layout, adaptive template download | || 2026-04-23 | **Allow duplicate No. NBA for Wajib & Sibuhar**: same member can have multiple transactions on different dates; duplicate detection now checks (No. NBA + Tanggal) combination for recurring types, while non-recurring types maintain strict uniqueness |
| 2026-04-24 | **Fix Simpanan import data extraction**: replace hardcoded row["No. NBA"] with normalized getCol helper; now correctly reads all column name variations (case/underscore) ensuring all Excel files are parsed properly |
| 2026-04-24 | **Fix Simpanan Excel import data extraction**: corrected column access to use normalizedMap (getCol helper); now properly reads all column name variations (case/underscore), ensuring data is correctly extracted from uploaded files |
...
| 2026-05-14 | **Database & Authentication Setup**: Prisma 7 with PostgreSQL schema (User, Account, Session, Anggota, Simpanan, Pinjaman, Angsuran, Transaksi, Pengeluaran); PrismaPg adapter; NextAuth with credentials provider; login page with demo account; middleware for route protection; external API integration route |